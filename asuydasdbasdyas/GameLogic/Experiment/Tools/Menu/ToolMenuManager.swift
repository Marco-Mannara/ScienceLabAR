...
class ToolMenuManager{
    ...
    init(_ experiment : Experiment){
        ...
        symbols["pickup"] = NodeLoader.loadModel("interaction_symbols/symbols","hand_symbol")!
        symbols["cancel"] = NodeLoader.loadModel("interaction_symbols/symbols","cancel_symbol")!
        symbols["return"] = NodeLoader.loadModel("interaction_symbols/symbols","return_symbol")!
        symbols["switch"] = NodeLoader.loadModel("interaction_symbols/symbols","onoff_symbol")!
        ...
        if heaterMenu == nil{
            ...
            heaterMenu?.addEntry(symbols["switch"]!.clone(), { (tool : Tool) in
                if let heater = tool as? Heater{
                    heater.toggleActive()
                    heater.state?.enter(StatePositioned.self)
                }
            })
