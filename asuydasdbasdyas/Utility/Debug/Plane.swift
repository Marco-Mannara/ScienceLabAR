...
*/
        ...
        if #available(iOS 12.0, *), ARPlaneAnchor.isClassificationSupported {
            ...
            let textNode = self.makeTextNode(classification, CGFloat(75))
            let areaValueNode = self.makeTextNode("0", CGFloat(60))
            
            classificationNode = textNode
            areaNode = areaValueNode
            ...
            extentNode.addChildNode(areaValueNode)
            
            areaValueNode.simdPosition = SIMD3<Float>(0,-0.05,0)
        }
        //#endif
    }
    ...
    private func makeTextNode(_ text: String, _ size : CGFloat) -> SCNNode {
        let textGeometry = SCNText(string: text, extrusionDepth: 1)
        textGeometry.font = UIFont(name: "Futura", size: size)
